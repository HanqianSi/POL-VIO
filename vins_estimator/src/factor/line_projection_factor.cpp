...
bool lineProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    double e1 = (obs_i(0)+obs_i(2))/2 * nc(0) + (obs_i(1)+obs_i(3))/2 * nc(1) + nc(2);
    double e2 = (obs_i(0)-obs_i(2)) * nc(0) +  (obs_i(1)-obs_i(3)) * nc(1);
    ...
    if (jacobians)
    {
        ...
        if(e2>0)
        jaco_e_l << ((obs_i(0)+obs_i(2))/2/l_sqrtnorm - nc(0) * e1 / l_trinorm ), ((obs_i(1)+obs_i(3))/2/l_sqrtnorm - nc(1) * e1 / l_trinorm ), 1.0/l_sqrtnorm,
                ((obs_i(0)-obs_i(2))/l_sqrtnorm - nc(0) * e2 / l_trinorm ), ((obs_i(1)-obs_i(3))/l_sqrtnorm - nc(1) * e2 / l_trinorm ), 0;
        else
        jaco_e_l << ((obs_i(0)+obs_i(2))/2/l_sqrtnorm - nc(0) * e1 / l_trinorm ), ((obs_i(1)+obs_i(3))/2/l_sqrtnorm - nc(1) * e1 / l_trinorm ), 1.0/l_sqrtnorm,
                -((obs_i(0)-obs_i(2))/l_sqrtnorm - nc(0) * e2 / l_trinorm ), -((obs_i(1)-obs_i(3))/l_sqrtnorm - nc(1) * e2 / l_trinorm ), 0;
