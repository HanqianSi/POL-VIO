...
//        std::cout << feature_buf.front()->header.stamp.toSec() << " " << feature_buf.back()->header.stamp.toSec() << "\n";
        ...
        if((linefeature_buf.front()->header.stamp > feature_buf.front()->header.stamp))
        {
            feature_buf.pop();
            continue;
        }
        ...
        linefeature_buf.pop();
        // ROS_WARN("header:%f,%f",img_msg->header.stamp.toSec(),linefeature_msg->header.stamp.toSec());
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_image = n.subscribe("/line_feature_tracker/feature", 2000, feature_callback);
    ros::Subscriber sub_linefeature = n.subscribe("/line_feature_tracker/linefeature", 2000, linefeature_callback);
