                ...
                Eigen::Matrix3d loop_correct_r)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        ofstream foutC1("/home/shq/dataset/SLAM/EuRoC/tra/POL-VIO/stamped_traj_estimate0.txt", ios::app);
        foutC1.setf(ios::fixed, ios::floatfield);
        foutC1.precision(9);
        foutC1 << header.stamp.toSec() << " ";
        foutC1.precision(5);
        foutC1 << odometry.pose.pose.position.x << " "
...
double reprojection_error_2( Vector4d obs, Matrix3d Rwc, Vector3d twc, Vector6d line_w ) {
            ...
            && it_per_id.is_triangulation == true )
        {
            int imu_i = it_per_id.start_frame;
             int imu_j = imu_i -1;
              ...
              Eigen::Vector3d twc = estimator.Ps[imu_i] + estimator.Rs[imu_i] * estimator.tic[0];   // twc = Rwi * tic + twi
        Eigen::Matrix3d Rwc = estimator.Rs[imu_i] * estimator.ric[0];               // Rwc = Rwi * Ric
...
//                  double allerr = 0;
        ...
        double ave = 0;
            ...
            for (auto &it_per_frame : it_per_id.linefeature_per_frame)   // 遍历所有的观测， 注意 start_frame 也会被遍历
            {
                ...
                Eigen::Vector3d t1 = estimator.Ps[imu_j] + estimator.Rs[imu_j] * estimator.tic[0];
                Eigen::Matrix3d R1 = estimator.Rs[imu_j] * estimator.ric[0];

                double err =  reprojection_error(obs, R1, t1, line_w);
                double err2 =  reprojection_error_2(obs, R1, t1, line_w);
                ave += err;
                i++;

                if(allerr2 < err2)    // 记录最大投影误差，如果最大的投影误差比较大，那就说明有outlier
                    allerr2 = err2;
    ...
    //        allerr = allerr / i;
            if (ave<0.9999||allerr2>3.0/500||1-(1-ave)*(e1-e2).norm()<0.99993)
            {
                continue;
            }
