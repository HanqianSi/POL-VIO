...
bool Estimator::visualInitialAlign()
{
    ...
    for (auto &it_per_id : f_manager.linefeature)
    {
        ...
        Eigen::Vector3d t0 = rot_diff*Ps[imu_i] + rot_diff*Rs[imu_i] * tic[0];   // twc = Rwi * tic + twi
        Eigen::Matrix3d R0 = rot_diff*Rs[imu_i] * ric[0];               // Rwc = Rwi * Ric
        ...
        for (auto &it_per_frame : it_per_id.linefeature_per_frame)   // 遍历所有的观测， 注意 start_frame 也会被遍历
        {
            ...
            Eigen::Vector3d t1 = rot_diff*Ps[imu_j] + rot_diff*Rs[imu_j] * tic[0];
            Eigen::Matrix3d R1 = rot_diff*Rs[imu_j] * ric[0];
        ...
        double allerr = 0;
        double ave = 0;
        Vector6d line_w = plk_to_pose(plk, R0, t0);
        imu_j = imu_i-1;
        int i_cnt=0;
        Eigen::Vector4d obs;
        for (auto &it_per_frame : it_per_id.linefeature_per_frame)   // 遍历所有的观测， 注意 start_frame 也会被遍历
        {
            ...
            Eigen::Vector3d t1 = rot_diff*Ps[imu_j] + rot_diff*Rs[imu_j] * tic[0];
            Eigen::Matrix3d R1 = rot_diff*Rs[imu_j] * ric[0];
            double err =  f_manager.reprojection_error(obs, R1, t1, line_w);
            ave+=err;
